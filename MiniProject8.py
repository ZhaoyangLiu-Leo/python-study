...
def rock_spawner():
    global lives
    if len(rock_group) < ROCK_NUMBER and started:
        score_vel = [0, 0]
        if random.choice([True, False]):
            score_vel[0] = score / 10.0
        else:
            score_vel[0] = -score / 10.0
        if random.choice([True, False]):
            score_vel[1] = score / 10.0
        else:
            score_vel[1] = -score / 10.0
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        rock_vel = [random.random() * .6 - .3 + score_vel[0], random.random() * .6 - .3 + score_vel[1]]
...
# draw the group of rock or missile
